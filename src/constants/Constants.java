package constants;

public class Constants {
    public final static String FINISHED_MAINTENANCE_FILENAME = "logfiles/finished_maintenance.txt";
    public final static int CAR_LOWER_TRAVEL_LIMIT = 3;
    public final static int CAR_UPPER_TRAVEL_LIMIT = 18;
    public final static int MAINTENANCE_TIME = 50;
    public final static int POLLUTION_DIESEL_ENGINE_LIMIT = 4000;
    public final static int POLLUTION_PETROL_ENGINE_LIMIT = 5000;
    public final static int POLLUTION_BIRD_LIMIT = 4000;
    public final static int BIRD_SING_COOLDOWN = 4000;
    public final static int ICE_CARS_IN_TOWN_LIMIT = 70;
    public final static boolean CONSOLE_LOG_ON = false;
    public final static boolean CONSOLE_RED_LOG_ON = false;
    public final static boolean BIRD_SING_CITY_MONITORING_INFO = true;
}
